...
class VisualizerWindowController: NSWindowController {
    ...
    func setupUI() {
        ...
        // 1. Export PDF
        let pdfBtn = NSButton(title: "📄 Exporter en PDF", target: self, action: #selector(exportPDFClicked))
        pdfBtn.bezelStyle = .rounded
        pdfBtn.frame = NSRect(x: 0, y: 6, width: 145, height: 30)
        bottomBar.addSubview(pdfBtn)

        // 2. Export TXT
        let txtBtn = NSButton(title: "📝 Exporter en TXT", target: self, action: #selector(exportTXTClicked))
        txtBtn.bezelStyle = .rounded
        txtBtn.frame = NSRect(x: 155, y: 6, width: 145, height: 30)
        bottomBar.addSubview(txtBtn)

        // 3. Copy All
        let copyBtn = NSButton(title: "📋 Copier Tout", target: self, action: #selector(copyAllClicked))
        copyBtn.bezelStyle = .rounded
        copyBtn.frame = NSRect(x: 310, y: 6, width: 125, height: 30)
        bottomBar.addSubview(copyBtn)

        // 4. Paste to App
        let pasteBtn = NSButton(title: "🚀 Injecter dans Antigravity", target: self, action: #selector(injectToAntigravityClicked))
    ...
    func showWithText(_ text: String, duration: String) {
        ...
        statsLabel.stringValue = "⏱️ Durée enregistrée : \(duration)   •   📊 Total : \(wordCount) mots   •   Dossier d'export : Bureau/TRANSCRIPTIONS_FEELPROD"
        showWindow(nil)
        window?.makeKeyAndOrderFront(nil)
        NSApp.activate(ignoringOtherApps: true)
    }

    @objc func exportPDFClicked() {
        let text = textView.string
        runExporter(mode: "--pdf", text: text)
    }

    @objc func exportTXTClicked() {
        let text = textView.string
        runExporter(mode: "--txt", text: text)
    }

    @objc func copyAllClicked() {
        NSPasteboard.general.clearContents()
        NSPasteboard.general.setString(textView.string, forType: .string)
        NSSound(named: "Glass")?.play()
    }

    @objc func injectToAntigravityClicked() {
        let text = textView.string
        NSPasteboard.general.clearContents()
    ...
    func runExporter(mode: String, text: String) {
        let script = Bundle.main.bundleURL.appendingPathComponent("Contents/Resources/exporter.py").path
        let proc = Process()
        proc.executableURL = URL(fileURLWithPath: "/usr/bin/python3")
        proc.arguments = [script, mode, text, currentDuration]
        let pipe = Pipe()
...
class PillView: NSView {
    ...
    func setupUI(width: CGFloat, height: CGFloat) {
        // 1. Guillaume Avatar on the Left (58x58 px)
        ...
        if let resURL = Bundle.main.resourceURL?.appendingPathComponent("avatar.png"),
           let img = NSImage(contentsOf: resURL) {
            avatarImageView.image = img
            avatarLoaded = true
        }
        if !avatarLoaded {
            let fallbackPath = "/Users/guillaumephilippe/.gemini/antigravity/brain/1d49e8d7-553d-4b83-b3f4-d8f61cdb3385/scratch/avatar_guillaume_nogreen.png"
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        if targetPopup.frame.contains(loc) || closeButton.frame.contains(loc) {
    ...
    func setState(rec: Bool, trans: Bool) {
        ...
        if trans {
            ...
            timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
                ...
                if h > 0 {
                    self.titleLabel.stringValue = String(format: "🔴 %02d:%02d:%02d • En écoute", h, m, s)
                } else {
                    self.titleLabel.stringValue = String(format: "🔴 %02d:%02d • En écoute", m, s)
                }
            }
        } else {
            timer?.invalidate()
...
class AppDelegate: NSObject, NSApplicationDelegate, AVAudioRecorderDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        pillView.onToggle = { [weak self] in
            self?.toggle()
        }
        pillView.onClose = { [weak self] in
            self?.quitApp()
        }
        pillView.onTargetChange = { [weak self] title in
            if title.contains("Visualiser") {
                ...
                self?.selectedTargetApp = "Antigravity"
            } else if title.contains("Chrome") {
                self?.selectedTargetApp = "Google Chrome"
            } else if title.contains("Safari") {
                self?.selectedTargetApp = "Safari"
            } else if title.contains("Notes") {
                self?.selectedTargetApp = "Notes"
            } else if title.contains("Presse-papier") {
                self?.selectedTargetApp = "Clipboard"
            } else {
                self?.selectedTargetApp = "Auto"
            }
        }
    ...
    func stopAndTranscribe() {
        ...
        let s = totalSec % 60
        if h > 0 {
            lastRecordedDurationString = String(format: "%02d:%02d:%02d", h, m, s)
        } else {
            lastRecordedDurationString = String(format: "%02d:%02d", m, s)
        }
        ...
        let durationStr = self.lastRecordedDurationString

        DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + 0.15) { [weak self] in
            guard let self = self else { return }
            let script = Bundle.main.bundleURL.appendingPathComponent("Contents/Resources/transcribe_engine.py").path
            ...
            proc.arguments = [script, self.audioURL.path, "--no-paste"]
            ...
            if !outputText.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    if target == "Visualizer" {
                        self.visualizer.showWithText(outputText, duration: durationStr)
